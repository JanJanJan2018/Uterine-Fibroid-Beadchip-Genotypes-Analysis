...
find25genes <- function(protein){
  ...
  proteinName <- as.data.frame(rep(protein,length(Protein$proteinType)))
...
getSummaries <- function(gene,protein){
  ...
  proteinName <- as.data.frame(rep(protein,length(Entrez1$EntrezSummary)))
  colnames(proteinName) <- 'proteinSearched'
  
  gene <- as.data.frame(rep(gene,length(Entrez1$EntrezSummary)))
  colnames(gene) <- 'gene'
